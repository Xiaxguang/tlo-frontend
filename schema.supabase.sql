-- Supabase / PostgreSQL schema for T-LO short-term migration
-- UID + password remains in profiles/players handled by Railway backend.
...
CREATE INDEX IF NOT EXISTS idx_players_uid_lower ON players(LOWER(uid));
CREATE INDEX IF NOT EXISTS idx_players_display_name_lower ON players(LOWER(display_name));
...
-- Keep sequences aligned after manual/imported IDs.
-- The import script also calls setval() after copying live data.
